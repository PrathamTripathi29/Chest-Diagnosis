...
from report_generator import load_report_model, generate_radiology_report
...
model               = None
report_model_loaded = False


def _load_model_sync():
    global model, report_model_loaded
    print("Loading classification model...")
    model = load_model()
    print("Loading report generation model...")
    load_report_model()
    report_model_loaded = True
...
async def root():
    return {
        ...
        "report_model_loaded": report_model_loaded,
    }
...
async def analyze(
    ...
    if model is None:
        raise HTTPException(
            503, "Model is still loading. Please wait 30 seconds and try again."
        )
    ...
    # Generate Grad-CAM heatmaps for real findings only
    ...
    # Generate radiology report text from image
    # Falls back gracefully if report model not loaded yet
    generated_report = None
    if report_model_loaded:
        generated_report = generate_radiology_report(image_bytes, predictions)

    # Build PDF with generated report included
    pdf_bytes, scan_id = build_report(
        predictions,
        heatmaps,
